...
def _call_vlm(prompt: str, image_b64: str, max_tokens: int = 4096) -> str:
    ...
    try:
        ...
        # Remove trailing /v1 suffix if present to avoid duplicate /v1/
        if base_url.endswith("/v1"):
            base_url = base_url[:-3]
        url = f"{base_url}/v1/chat/completions"

        logger.debug(f"Calling VLM at {url} with model {config.vlm_model or 'qwen3-vl-30b'}")

        payload = {
            "model": config.vlm_model or "qwen3-vl-30b",
            "messages": [
                {
                    "role": "user",
                    "content": [
        ...
        try:
            ...
            if "<html" in raw_text.lower() or "<!doctype" in raw_text.lower():
                raise ValueError(
                    ...
                    f"Please check that '{config.vlm_model or 'qwen3-vl-30b'}' "
        ...
        if "choices" not in result:
            logger.error(f"VLM response missing 'choices' key. Keys: {list(result.keys())}")
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            for i in range(actual_page_count):
                ...
                if analyze_structure and i == 0:
                    ...
                    structure = analyze_document_structure.invoke({"image_path": str(temp_path)})
