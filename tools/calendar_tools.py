...
def get_upcoming_events(days: int = 7) -> str:
    ...
    with get_connection() as conn:
        ...
        for row in results:
            ...
            formatted.append(f"{status_icon} {row['start_time']}: {row['title']}")
            ...
            if row["attendees"]:
                try:
                    ...
                    logger.warning(
                        f"Failed to parse attendees for event {row.get('title', 'unknown')}: {e}"
                    )
...
def create_event(
    ...
    with get_connection() as conn:
        ...
        conn.execute(
            ...
            (id, chat_id, title, description, start_time, end_time, location, attendees, status)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'confirmed')
        """,
            (
                event_id,
...
def approve_proposal(proposal_id: int) -> str:
    ...
    try:
        ...
        if conn:
            conn.rollback()
        wrapped = wrap_exception(e, DatabaseError, operation="proposal_approval")
        logger.error(f"Database error during proposal approval: {wrapped}")
        return f"Approval failed due to database error: {str(e)}"
    except Exception as e:
        if conn:
            conn.rollback()
        wrapped = wrap_exception(e, ExternalServiceError, operation="proposal_approval")
        logger.error(f"Unexpected error during proposal approval: {wrapped}")
...
def reject_proposal(proposal_id: str, reason: str = "") -> str:
    ...
    Returns:
        Success message
    """
    print(f"[REJECT] Proposal {proposal_id}...")
    ...
    with get_connection() as conn:
        ...
        print(f"[OK] Proposal rejected: {title}")
...
def update_event(event_id: str, updates: dict) -> str:
    ...
    with get_connection() as conn:
        ...
        print(f"[OK] Event updated: {updated_fields}")
